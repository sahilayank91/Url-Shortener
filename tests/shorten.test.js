...
test("different URLs should yield different shortcodes", () => {
  const shortenedPathA = shortenAndLog("http://example.com/ab");
  const shortenedPathB = shortenAndLog("http://example.com/ba");
  const shortenedSubdomainA = shortenAndLog("http://ab.example.com");
  const shortenedSubdomainB = shortenAndLog("http://ba.example.com");
